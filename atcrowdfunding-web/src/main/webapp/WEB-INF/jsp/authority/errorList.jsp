    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
          <div><a class="navbar-brand" style="font-size:32px;" href="#">智能身份授权系统- 审核未通过列表</a></div>
        </div>
	...
	<script src="${APP_PATH}/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            function pageQuery( pageno ) {
            	...
            	$.ajax({
            		type : "POST",
            		url  : "${APP_PATH}/authorityCheck/errorList",
            		data : jsonData,           		
            		beforeSend : function(){
            			...
            			if ( result.success ) {
            				...
            				$.each(tacs, function(i, tac){
            	                tableContent += '<tr>';
	          	                ...
	          	                tableContent += '  <td>'+tac.receptname+'</td>';
	          	                tableContent += '  <td>'+tac.lockname+'</td>';
	          	                tableContent += '  <td>'+tac.visitecause+'</td>';
	          	                tableContent += '  <td>'+tac.applytime+'</td>';
	          	                tableContent += '  <td>';
	          					tableContent += '	  <button type="button" onclick="deleteUser('+tac.id+', \''+tac.username+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
	          					tableContent += '  </td>';
	          	                tableContent += '</tr>';
            				});
